...
typedef struct Celula {
	Game elem;        // Elemento inserido na celula.
	struct Celula* prox; // Aponta a celula prox.
} Celula;

Celula* novaCelula(Game jogo) {

   Celula* novaCell = (Celula*) malloc(sizeof(Celula));
...
void inserir(Game jogo) {
    topo->prox = novaCelula(jogo);
    topo = topo->prox;
}
...
int main() {
    ...
    for (int i = 0; i < quant; i++){
        ...
        char acao;
        int app_id;
        scanf("%c %d", &acao, &app_id);

        if(acao == 'I'){

            int j = 0;
            for(j = 0; jogos[j].app_id != app_id; j++);
            inserir(jogos[j]);
            printf("I\n");

        }else if(acao == 'R'){

            Game retorno = remover();
            printf("(R) %s\n", retorno.name);
