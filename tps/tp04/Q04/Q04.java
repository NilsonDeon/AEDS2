...
class Q04 {
    ...
    public static void main(String args[]) throws Exception, ParseException {
        ...
        int[] compMov = { 0, 0 }; // index 0 = comparacoes entre arrays | index 1 = movimentacoes do array

        String valores = "", str = "";
        int i = 0, id = 0, tamanho = 0;
        ...
        OutputStream os = new FileOutputStream("781626_quicksort2.txt");
        Writer wr = new OutputStreamWriter(os); // criação de um escritor
        BufferedWriter br = new BufferedWriter(wr); // adiciono a um escritor de buffer

        br.write("781626\t" + compMov[0] + "\t" + compMov[1] + "\t" + tempo + "s.");
...
class Cell {
    ...
    public Cell(Games jogo) {
        this.elem = jogo;
        this.prev = this.prox = null;
    }
}
...
class ListaDuplamenteEncadeada {
    ...
    public void inserir(Games jogo, int pos) throws Exception {
        ...
        if (pos < 0 || pos > tamanho()) {
            ...
            // vai ate a posicao anterior da insercao
            ...
            aux.prev.prox = aux.prox.prev = aux;
    ...
    public Games removerInicio() throws Exception {
        ...
        Cell aux = inicio;
        inicio = inicio.prox;
        Games resp = inicio.elem;
        aux.prox = inicio.prev = null;
        aux = null;
    ...
    public int tamanho() {
        int tamanho = 0;
        for (Cell i = inicio; i != fim; i = i.prox, tamanho++)
            ;
        return tamanho;
    }
    ...
    public Cell posCell(int pos) {

        Cell resp = inicio.prox;
        for (int i = 0; i < pos; i++, resp = resp.prox);

        return resp;
    }

    void sort(int esq, int dir, int[] compMov){
        ...
        while (i <= j){
            ...
            while ( comparaData(posCell(pivo).elem, posCell(i).elem) ){
                
                compMov[0] += 2; 
                i++;
            }
            while ( comparaData(posCell(j).elem, posCell(pivo).elem) ){
    ...
    public boolean comparaData(Games i, Games j) {
        boolean resp;

        if (i.getRelease_date().compareTo(j.getRelease_date()) == 0){
            resp = (i.getName().compareTo(j.getName()) > 0);
        } else {
            resp = (i.getRelease_date().compareTo(j.getRelease_date()) > 0);
        }

        return resp;
    }

    void swap(int posA, int posB, int[] compMov) {

        compMov[1] += 3;

        Games aux = posCell(posA).elem;
        posCell(posA).elem = posCell(posB).elem;
        posCell(posB).elem = aux;
    }

    public void impirmirLista(){

        for(Cell i = inicio.prox; i != null; i = i.prox){
            i.elem.imprimir();
        }
...
class Games {
    ...
    public Games(int app_id, String name, Date release_date, String owners, int age, float price, int dlcs,
    ...
    public void imprimir() {
        String mostrar = app_id + " " + name + " " + trataData(release_date) + " " + owners + " " + age + " "
                + trataPrice(price) + " " + dlcs + trataArr(languages) + " " + website + " " + windows + " " + mac + " "
                + linux + " " + trataUpVote(upvotes) + " " + trataPlayTime(avg_pt) + " " + developers
                + trataArr(genres);
