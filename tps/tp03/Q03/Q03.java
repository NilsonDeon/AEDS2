...
class Q03{
    ...
    // inserindo entrada do usuario na lista
        ...
        int[] counts = lista.ordenaInsercaoLista();
        lista.impirmirLista();
        ...
        OutputStream os = new FileOutputStream("781626_insercao.txt"); 
...
class Lista{
    ...
    public int[] ordenaInsercaoLista(){
        
        int countComp = 1, countMov = 0, i, j;

        Games aux = new Games();

        for (i = 1; i < countLista; i++) {

            aux = jogos[i]; 
            j = (i - 1); 
            
            while ( j >= 0 && ( jogos[j].getApp_id() > aux.getApp_id() ) ){ 
                countComp++;
