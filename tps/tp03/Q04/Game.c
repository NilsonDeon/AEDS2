...
void shellSort(Game jogos[], int tamanho, int compMov[]){
    ...
    for (int i = gap; i > 0; i /= 2){
        ...
        for (int j = i; j < tamanho; j += 1){
            ...
            int k;
            compMov[0] += 1;
            for (k = j; k >= i && (jogos[k - i].app_id > aux.app_id); k -= i){
                compMov[1] +=1;
                jogos[k] = jogos[k - i];
            }
...
int main() {
    ...
    fp2 = fopen("781626_shellsort.txt", "w");
