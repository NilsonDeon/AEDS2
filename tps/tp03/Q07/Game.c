...
void countingSort(Game *jogos, int tamanho, int compMov[]){

    // 100% é o maior valor possivel
    int tamCount = 101;
    int count[tamCount];
    Game ordenado[tamanho];

    // preenchendo count de 0
    for (int i = 0; i < tamCount; count[i] = 0, i++);

    // contando recorrecia de valor
    for (int i = 0; i < tamanho; i++){
        compMov[0] += 1;
        int aux = jogos[i].upvotes;
        count[aux]++;
    }

    // num de elementos menores ou iguais a i
    ...
    for(int i = (tamanho - 1); i >= 0; i--){
        ...
        compMov[1] += 1;
        int aux = jogos[i].upvotes;
        ordenado[ (count[aux]-1) ] = jogos[i];
        count[aux]--;

    }

    // adicionando valores ordenados no array de jogos
    for(int i = 0; i < tamanho; jogos[i] = ordenado[i], compMov[1] += 1, i++);
...
int main() {
    ...
    countingSort(jogos, count, compMov);
    ...
    fp2 = fopen("781626_countingsort.txt", "w");

    int comparacoes = compMov[0];
    int movimentacoes = compMov[1];

    fprintf(fp2, "%i\t%i\t%lf s.", comparacoes, movimentacoes, total);
