...
void swap(int pos1, int pos2, Game jogos[], int compMov[]){
...
void selecaoRec(Game jogos[], int contador, int i, int compMov[]){

    int min = i;
    for (int j = i + 1; j < contador; j++){
        compMov[0] += 1;
        int resultado = strcmp(jogos[min].name, jogos[j].name);
        if (resultado > 0){
            min = j;
        }
    }
    
    swap(min, i, jogos, compMov);

    if (i++ < contador){
        selecaoRec(jogos, contador, i++, compMov);
    }
}
...
int main() {
    ...
    selecaoRec(jogos, count, 0, compMov);
    ...
    fp2 = fopen("781626_selecaoRecursiva.txt", "w");
